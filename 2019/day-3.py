...
def getArry(operationsArray):
    ...
    for operations in operationsArray:
        ...
        for i in range(0, length):
            ...
            if i > 0:   # We are operating on a change in direction, so let's indicate that it's a change
                mapChar = "+"
                matrix[currentX][-1+size-currentY] = mapChar

            isUp = op.startswith("U")
            isDown = op.startswith("D")
            isLeft = op.startswith("L") 
            isRight = op.startswith("R")
            deltaValueStr = op.replace("U", "").replace("D", "").replace("L", "").replace("R", "")
            deltaValue = int(deltaValueStr)

            if (isUp or isDown):            
                for d in range(0, deltaValue):                                        
                    if isUp:                    
                        currentY = currentY + 1
                    if isDown:
                        currentY = currentY - 1
                    mapChar = "|"
                    existingChar = matrix[currentX][-1+size-currentY] 
                    cableIntersectionCount = myPoints.count("{},{}".format(currentX, currentY))
                    
                    if existingChar == "-" and cableIntersectionCount < 1: 
                        mapChar = "X"
                        distance = abs(currentX-START_X) + abs(currentY-START_Y)
                        print("{}".format(distance))

                    matrix[currentX][-1+size-currentY] = mapChar
                    myPoints.append("{},{}".format(currentX, currentY))
            ...
            if (isLeft or isRight):
                for d in range(0, deltaValue):                    
                    ...
                    #print ("setting map char to be {} at {},{}").format(mapChar, currentX, currentY)

            
    return matrix

def getDistances(arry):
    distances = list()
    distance = 0
    for y in range(0, len(arry)):
        size = len(arry[y])
        for x in range(0, size):
            translatedY = -1+size-y                    
            if arry[x][translatedY] == "X":
...
operationsArry = list()
rawOperations = raw.split("\n")
operationsArry.append(list(rawOperations[0].split(",")))
operationsArry.append(list(rawOperations[1].split(",")))

arry = getArry(operationsArry)
#drawArry(arry)
#distances = getDistances(arry)
#print("Distances: {}, Min: {}").format(distances, min(distances))
